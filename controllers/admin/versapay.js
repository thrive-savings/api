module.exports = (User, Account, Queue, request, Bluebird) => ({
  sync: {
    schema: [['data', true, [['userID', 'integer']]]],
    async method (ctx) {
      const { data: { userID: providedUserID } } = ctx.request.body

      const whereClause = { processed: false }
      if (providedUserID) { whereClause.userID = providedUserID }
      const queues = await Queue.findAll({ where: whereClause })

      queues.forEach(async (queue) => {
        const { type, amount, userID, accountID, transactionReference, uuid } = queue
        ...
        if (type === 'credit' && user.balance < amount) {
          ...
          if (accountID) {
            ...
            account = await Account.findOne({ where: { userID, isDefault: true } })
